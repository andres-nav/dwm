...
enum { SchemeNorm, SchemeSel, SchemeHid, SchemeNormUnfocus, SchemeSelUnfocus }; /* color schemes */
...
drawbar(Monitor *m)
{
	...
	int color = SchemeNorm;
	int color_sel = SchemeSel;
	if ((m != selmon) && (mons->next)) { // Not focus monitor
		color = SchemeNormUnfocus;
		color_sel = SchemeSelUnfocus;
	} 
	...
	if (m == &mons[mainmon]) { /* status is only drawn on main monitor */
		...
		unsigned int lp = 10;
		tw = TEXTW(stext) - lrpad + 2 + lp; /* 2px right padding 4px left padding */
		drw_text(drw, m->ww - tw, 0, tw, bh, 2 + lp, stext, 0);
	}
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		if(!(occ & 1 << i || m->tagset[m->seltags] & 1 << i))
			continue;
		w = TEXTW(tags[i]);
		drw_setscheme(drw, scheme[m->tagset[m->seltags] & 1 << i ? color_sel : color]);
